...
<xsl:stylesheet version="2.0"
    ...
    <!-- superclasse owl:Thing (sempre dichiarata) generata automaticamente -->
    ...
    <!-- document element of initial graphml file -->
    ...
    <!-- header of generated result files -->
<!--    <xsl:variable name="result-header" as="xs:string" select="concat(
        ' Generated by ', system-property('xsl:vendor'), ' ', system-property('xsl:version'), 
        ' XSLT processor (', system-property('xsl:vendor-url'), ') ')"/>-->
    
    <!-- dato un URI restituisce una stringa utilizzabile come filename basata su di esso -->
<!--    <xsl:function name="utls:uri-to-filename" as="xs:string">
    ...
    <xsl:function name="utls:get-entities-from-manchester-string" as="node()*">
        ...
        <!-- decomponi la stringa Manchester in token -->
        ...
        <!-- test tokens separati da spazi -->
        <!--<xsl:element name="manchester" namespace="{$rtf-namespace}">
            <xsl:attribute name="name" select="string-join($tokens, ' ')"/>
        </xsl:element>-->
    ...
    <xsl:function name="utls:scan-manchester-tokens" as="node()*">
        ...
        <!-- la chiamata effettua qualcosa solo se esiste il token corrente -->
        <xsl:if test="exists($token)">
            
            <!-- aggiorna il contesto in base al contesto attuale e al token corrente -->
            <xsl:variable name="updated-context" as="xs:string*" select="utls:update-context($context, $token)"/>
            
            <!-- determina il tipo del token corrente -->
            ...
            <!-- se il token corrente è o potrebbe essere un'entità, genera un nodo xml corrispondente -->
            ...
            <!-- richiamati ricorsivamente sul prossimo token -->
    ...
    <xsl:function name="utls:is-followed-by-typed-properties" as="xs:boolean">
        <xsl:param name="property-type" as="xs:string"/>    <!-- 'object-property' or 'data-property' -->
        <xsl:param name="tokens" as="xs:string*"/>
        <xsl:param name="direction" as="xs:string"/>        <!-- 'forward' or 'backward' scan in tokens -->
        <xsl:param name="rtf" as="node()"/>
        <xsl:param name="ontology-id" as="xs:string"/>
    ...
    <!-- per ogni stringa Manchester, inserisce nelle property restriction 'min' 'max' 'exactly' 
        le primary 'owl:Thing' e 'rdfs:Literal' se non presenti (la sintassi Manchester 
        non richiede la loro presenza, il validatore della Manchester university e Protégé invece sì) -->
    ...
    <!--
       prelevo tutte le classi dell'ontologia classes = A1, A2, A3, ...
       prelevo tutte le classi radici root-classes = Ai, Ak, Aj, ... 
       
       (ottieni una lista di classi radici disgiunte disjoint-classes, ossia appartenenti a gerarchie diverse)
       per ogni classe radice R1 in root-classes
           per ogni classe radice R2 precedente R1 nella sequenza
               se R1 appartiene alla stessa gerarchia di R2
                   rimuove R1 dalla sequenza
                   
       
        (ottieni albero che definisce le gerarchie)
        per ogni classe radice R1 in disjoint-classes
            genero un elemento hierarchy
            per ogni classe radice R2 in root-classes
                sort root-classes
                se R2 appartiene alla stessa gerarchia di R1
                    copio R2 nell'elemento hierarchy corrente
    -->
    <xsl:function name="utls:split-root-classes-in-different-hierarchies" as="node()">
        ...
        <xsl:variable name="root-classes" as="node()*" 
            select="$rtf/rtf:&class;[utls:contains-id(@ontology-id, $ontology-id)][@id (: esclude classi auto-generate :)]
                [not(@id = $rtf/rtf:&axiom;[@name eq '&subclassof-axiom;']/@source-id)]"/>
        ...
        <xsl:element name="class-hierarchies" namespace="{$rtf-namespace}">
            ...
            <!-- per ogni gerarchia, rappresentata da una classe radice, genero un elemento hierarchy
                contenente tutte le classi radici che le appartengono -->
            <xsl:for-each select="$disjoint-root-classes">
                ...
                <xsl:element name="hierarchy" namespace="{$rtf-namespace}">
                    ...
                    <!-- inserisco ordinatamente le classi radici che appartengono alla gerarchia corrente -->
    ...
    <xsl:function name="utls:disjoin-classes" as="node()*">
        ...
        <xsl:sequence select="
            (: se non ci sono più classi da analizzare restituisci le classi accumulate :)
            if (empty($classes)) then $disjoint-classes
            (: altrimenti richiamati ricorsivamente sulle rimanenti classi :)
            else utls:disjoin-classes($sub-class-axioms, remove($classes, 1),
                (: la classe corrente viene accumulata se non appartiene alla gerarchia di alcuna delle classi già accumulate :)
    ...
    <xsl:function name="utls:contains-id" as="xs:boolean">
        ...
        <xsl:sequence select="utls:contains-word($str,$id)"/>
    </xsl:function>
    ...
    <xsl:function name="utls:add-ids" as="xs:string">
        ...
        <xsl:sequence select="utls:add-words($str,$ids)"/>
    </xsl:function>
    
    <xsl:function name="utls:remove-id" as="xs:string">
        <xsl:param name="str" as="xs:string"/>
        <xsl:param name="id" as="xs:string"/>
        
        <xsl:sequence select="utls:remove-word($str,$id)"/>
    </xsl:function>
    

    <xsl:function name="utls:contains-word" as="xs:boolean">
        <xsl:param name="str" as="xs:string?"/>
        <xsl:param name="word" as="xs:string"/>
        
        <xsl:sequence select="matches($str,concat('(^|\s)',$word,'(\s|$)'))"/>
    </xsl:function>
    
    <xsl:function name="utls:add-words" as="xs:string">
        <xsl:param name="str" as="xs:string"/>
        <xsl:param name="words" as="xs:string*"/>
        
        <xsl:variable name="not-contained-words" as="xs:string*" select="for $word in $words
            return if (utls:contains-word($str,$word)) then () else $word"/>
        
        <xsl:sequence select="string-join(($str,$not-contained-words),' ')"/>
    </xsl:function>
    
    <xsl:function name="utls:remove-word" as="xs:string">
        <xsl:param name="str" as="xs:string"/>
        <xsl:param name="word" as="xs:string"/>
        
        <xsl:variable name="words" as="xs:string*" select="tokenize($str,' ')"/>
        <xsl:variable name="filtered-words" as="xs:string*" select="for $item in $words
            return if ($item ne $word) then $item else ()"/>
        <xsl:sequence select="string-join($filtered-words,' ')"/>
    </xsl:function>
    ...
    <!-- genera l'id della nuova entità dopo il punning con l'assioma specificato -->
    ...
    <!-- data una stringa, ne effettua una ricerca all'interno dei sinonimi degli assiomi
        OWL2 in Manchester syntax (config.xml). Restituisce un nodo xml contenente 
        1) l'assioma (la string iniziale se non è stato trovato un riscontro negli assiomi OWL), 
        2) un booleano indicante se l'assioma è un assioma OWL 
        3) se l'assioma è un assioma OWL, il tipo del dominio e codominio attesi 
            per quell'assioma (punning) -->
    ...
    <!-- dato il nome di un'ontologia importata del tipo "IRI version-IRI"
        restituisce l'IRI da utilizzare nella clausola "Import" 
        (determinato dal valore del parametro booleano $use-imported-ontology-version-iri in config.xsl) -->
    ...
    <!-- dato un nome di un ontologia del tipo "IRI version-IRI", restituisce il primo IRI -->
    ...
    <!-- dato un nome di un ontologia del tipo "IRI version-IRI", restituisce l'IRI di versione 
        se esiste, altrimenti il primo IRI (IRI generale) -->
    ...
    <!-- it encloses the IRI in angular brackets -->
    ...
    <!-- dato il valore di una restrizione restituisce un IRI esplicativo univoco
        (univocità garantita dalla privatezza dell'IRI di base, 
        definito dal nodo "graffoo-prefix" in config.xml) -->
    <xsl:function name="utls:create-restriction-iri" as="xs:string">
        <xsl:param name="restriction-name" as="xs:string"/>
        
        <!-- racchiude i prefissi in appositi delimitatori (config.xml) -->
        <xsl:variable name="label-without-prefixes" as="xs:string"
            select="replace($restriction-name,'([^: \(]+):', 
            concat($restriction-label-prefix-left-delimiter,'$1', 
            $restriction-label-prefix-right-delimiter))"/>
        
        <!-- concatena i termini dell'etichetta con un apposito carattere (config.xml) -->
        <xsl:variable name="label-without-blank" as="xs:string" 
            select="replace($label-without-prefixes,'\s',$restriction-label-blank-substitute)"/>
        
        <xsl:sequence select="utls:bracket-iri(concat($graffoo-prefix/@uri,$label-without-blank))"/>
    </xsl:function>
    
    
    <!-- data una regola SWRL (Semantic Web Rule Language), la adatta alla Manchester syntax -->
    <!-- in particolare ogni parametro ?x viene tradotto in ?<urn:swrl#x> come avviene in Protégé -->
    ...
    <!-- dato il nome di un linguaggio per regole diverso da SWRL, restituisce l'IRI che definisce
        l'annotation property "has<Lang>Rule" per quel linguaggio -->
    ...
    <!-- verifica se un nodo è un prefixes box -->
    ...
    <!-- restituisce i prefissi del prefixes box (senza il ':') -->
    ...
    <!-- restituisce gli uri del prefixes box -->
    ...
    <!-- verifica se un nodo costituisce un'ontologia -->
    ...
    <!-- restituisce il nome dell'ontologia dal suo id -->
    <!-- se l'id si riferisce all'id di default, restituisce il nome di default -->
    <xsl:function name="utls:get-ontology-name-by-id" as="xs:string">
        <xsl:param name="id" as="xs:string"/>
        <xsl:sequence select="if ($id eq $default-ontology-id) then 
            $default-ontology-iri else utls:get-node-name($root//g:node[@id eq $id])"/>
    </xsl:function>
    
    <!-- restituisce l'id dell'ontologia cui appartiene il nodo/arco/ontologia del grafo graffoo -->
    <!-- se non appartiene a nessuna ontologia restituisce l'id di default -->
        ...
        <xsl:choose>
            ...
            <xsl:when test="$element[self::g:edge]">
                ...
                <xsl:variable name="source-node" as="node()" 
                    select="$root//g:node[@id eq utls:get-edge-source-id($element)]"/>
                ...
                <xsl:variable name="target-node" as="node()" 
                    select="$root//g:node[@id eq utls:get-edge-target-id($element)]"/>
    ...
    <!-- rtf utilizzato nella utls:is-ontology-reachable -->
    ...
    <!-- determina se un'ontologia destinazione è raggiungibile tramite clausole Import
        dall'ontologia sorgente -->
    ...
    <!-- verifica se un nodo costituisce una external rule -->
    ...
    <!-- verifica se un nodo è un datarange -->
    ...
    <!-- verifica se un nodo è un datarange composto -->
    ...
    <!-- verifica se un nodo è una classe -->
    ...
    <!-- verifica se un nodo è una restrizione -->
    ...
    <!-- verifica se un nodo è un'istanza di una classe -->
    ...
    <!-- verifica se un nodo è un letterale -->
    <!-- sono esclusi i nodi letterali utilizzati nei prefixes box -->
    ...
    <!-- verifica se un nodo costituisce un additional axiom -->
    ...
    <!-- verifica se un arco definisce una data property -->
    ...
    <!-- verifica se un arco definisce la dichiarazione di una data property facility
        (widget supporting understanding) -->
    ...
    <!-- restituisce l'id dell'elemento sorgente di una dataproperty -->
    ...
    <!-- restituisce l'id dell'elemento target di una dataproperty -->
    ...
    <!-- verifica se un arco definisce una object property -->
    ...
    <!-- verifica se un arco definisce la dichiarazione di una objectproperty
        (widget supporting understanding) -->
    ...
    <!-- restituisce l'id dell'elemento sorgente di una objectproperty -->
    ...
    <!-- restituisce l'id dell'elemento target di una objectproperty -->
    ...
    <!-- verifica se un arco definisce una annotationproperty -->
    ...
    <!-- verifica se un arco definisce la dichiarazione di una annotationproperty
        (widget supporting understanding) -->
    ...
    <!-- restituisce l'id dell'elemento sorgente di una annotationproperty -->
    ...
    <!-- restituisce l'id dell'elemento target di una annotationproperty -->
    ...
    <!-- verifica se un arco costituisce un'asserzione -->
    ...
    <!-- restituisce l'id dell'elemento sorgente di un'asserzione -->
    ...
    <!-- restituisce l'id dell'elemento target di un'asserzione -->
    ...
    <!-- verifica se un arco definisce una qualunque property -->
    ...
    <!-- restituisce l'id del nodo sorgente dell'arco -->
    ...
    <!-- restituisce l'id del nodo target dell'arco -->
    ...
    <!-- restituisce l'etichetta dei nodi specificati -->
    ...
    <!-- restituisce l'etichetta dei nodi con id specificati -->
    <xsl:function name="utls:get-node-name-by-id" as="xs:string*">
        <xsl:param name="ids" as="xs:string*"/>
        <xsl:sequence select="utls:get-node-name($root//g:node[@id = $ids])"/>
    </xsl:function>
    
    <!-- restituisce la stringa Manchester rappresentante l'etichetta
        dei nodi specificati private dagli eventuali commenti -->
    ...
    <!-- restituisce una stringa contenente le informazioni presentazionali del nodo:
        esempio: x=622.5 y=589.0 height=44.0 width=71.5 fill-color=#FFFF00 -->
    <!-- si assume che il nodo sia un nodo entità semplice (classe, datarange, individuo) -->
    ...
    <!--  in yEd (Properties -> Data -> Description) è possibile associare a ogni nodo/arco una stringa
        utilizzata nell'ontologia finale come valore delle annotation property rdfs:label e rdfs:comment
        dell'entità (la prima stringa non vuota costituisce la label, tutto ciò che segue 
        costituisce il comment) -->
    ...
    <!-- restituisce l'eventuale rdfs:label dell'entità -->
    ...
    <!-- restituisce l'eventuale rdfs:comment dell'entità -->
    ...
    <!-- un arco etichettato definisce al suo interno uno o più nodi EdgeLabel,
        ognuno dei quali definisce al suo interno una o più etichette separate da newline -->
    <!-- ogni nodo EdgeLabel viene identificato da una stringa ottenuta per concatenazione di
        * id dell'arco cui appartiene (attributo "id" del nodo "edge")
        * posizione dell'EdgeLabel rispetto agli eventuali fratelli EdgeLabel -->
    <!-- ogni etichetta viene identificata da una stringa ottenuta per concatenazione di
        * id dell'EdgeLabel cui appartiene
        * posizione dell'etichetta rispetto alle altre etichette all'interno dell'EdgeLabel -->
    
    
    <!-- data una stringa rappresentante il contenuto di un EdgeLabel, restituisce le etichette non vuote
        che la compongono -->
    ...
    <!-- restituisce un nodo xml che definisce un id per ogni etichetta dell'arco -->
    <xsl:function name="utls:generate-labels-and-ids-by-edge" as="node()">
        ...
        <xsl:variable name="result" as="node()">
            <xsl:element name="labels" namespace="{$rtf-namespace}">
                ...
                <!-- ogni edge può definire multipli edgelabel -->
                <xsl:for-each select="$edge/g:data[@key eq $edge-content]//y:EdgeLabel">
                    
                    <!-- indice dell'edgelabel corrente rispetto agli EdgeLabel fratelli -->
                    <xsl:variable name="edgelabel-index" as="xs:integer" select="position()"/>
                    
                    <!-- etichette definite per l'edgelabel corrente -->
                    <xsl:variable name="labels" as="xs:string*" 
                        select="utls:tokenize-edgelabel(current())"/>
                    
                    <!-- per ogni etichetta crea un nodo "label" con attributi "name" (l'etichetta)
                        e id (l'id associato a tale etichetta) -->
    ...
    <!-- restituisce un nodo xml che definisce un id per ogni etichetta dell'EdgeLabel -->
    ...
    <!-- dato un EdgeLabel, genera un id per esso -->
    ...
    <!-- dato l'id di un EdgeLabel, restituisce il corrispondente EdgeLabel del grafo graphml --> 
    <xsl:function name="utls:get-edgelabel-by-id" as="node()">
        ...
        <xsl:variable name="edge" as="node()" select="$root//g:edge[@id eq $edge-id]"/>
    ...
    <!-- dato l'id di un EdgeLabel, restituisce il corrispondente edge cui appartiene -->
    <xsl:function name="utls:get-edge-by-edgelabel-id" as="node()">
        <xsl:param name="id" as="xs:string"/>
        <xsl:sequence select="$root//g:edge[@id eq substring-before($id, $edgelabel-id-first-prefix)]"/>
    </xsl:function>
    ...
    <xsl:function name="utls:axiom-refers-to" as="xs:string+">
        ...
        <xsl:variable name="link-edges" as="node()*" 
            select="$root//g:edge[(@source,@target) = $axiom/@id]"/>
        ...
        <xsl:choose>
            ...
            <xsl:when test="exists($link-edges)">
                <xsl:sequence select="$root//g:node
                    [(@id = $link-edges/(@source|@target)) and (@id ne $axiom/@id)]/@id"/>
            </xsl:when>
    ...
    <xsl:function name="utls:get-edgelabel-position" as="node()">
        ...
        <xsl:variable name="source-node" as="node()" select="$root//g:node[@id eq $edge/@source]"/>
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:variable name="target-node-geometry" as="node()" select="
                        $root//g:node[@id eq $edge/@target]/g:data[@key eq $node-content]//y:Geometry"/>
...
                 is shitfted to the left) -->
    ...
    <!-- date due coppie di punti ((x1,y1),(x2,y2)) e ((x3,y3),(x4,y4)), calcola il punto di
        intersezione tra la rette passanti per la prima e la seconda coppia -->
    <!-- se le due rette non si intersecano o se una o entrambe le coppie di punti non definiscono
        una retta (punti coincidenti) restituisce una sequenza vuota -->
